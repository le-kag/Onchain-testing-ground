...
{   
    ...
    function getAttributes(bytes32 hash)
        public view returns (CercleAttributes memory)
    {
        Rand memory rand = Random.create(uint256(hash));
        CercleAttributes memory attributes = createAttributes(rand);
        return attributes;
    }
        ...
        public view returns(string memory)
    {
        ...
        SB.writeStr(b, "px;"
            ...
            "<g id='Cercle' shape-rendering='geometricPrecision' filter='url(#glow)'>");
    ...
    function createAttributes(Rand memory rand) 
        private view returns(CercleAttributes memory)
    {
        int weighted = int8(curve[uint(Random.range(rand, 0, 1023))]);
        
        CercleAttributes memory attributes;
...
contract CercleMetadata is Ownable
{   
    using Strings for uint256;

    CercleDrawing _drawing;
    string private _imageBaseUri;

    constructor(
        CercleDrawing drawing,
    ...
    function setDrawing(CercleDrawing drawing)
        public onlyOwner
    {
        _drawing = drawing;
    }
    ...
    {
        CercleAttributes memory attributes = _drawing.getAttributes(hash);
...
contract Cercle is ERC721Enumerable, Ownable
{
    ...
    CercleDrawing private _drawing;
    CercleMetadata private _metadata;
    ...
    function setDrawingContract(CercleDrawing drawing)
        public onlyOwner isUnlocked
    {
        _drawing = drawing;
    }

    function setMetadataContract(CercleMetadata meta)
        public onlyOwner
    {
        _metadata = meta;
    }
